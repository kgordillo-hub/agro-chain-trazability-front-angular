...
export class MyApp {
  ...
  private unreadCountObservable: any = new ReplaySubject<number>(0);
	...
	private initializeOptions(): void {
		...
		this.options.push({
			iconName: 'search',
			displayName: 'Consultar estado producto',
			component: DetailsPage
		});

		this.options.push({
			iconName: 'document',
			displayName: 'Consultar ordenes compra',
			component: DetailsPage
		});

    this.options.push({
			iconName: 'send',
			displayName: 'Emitir respuesta orden compra',
		...
		this.options.push({
			...
			subItems: [
				...
				{
					iconName: 'bookmark',
					displayName: 'Agregar estado a producto',
					component: DetailsPage
				}
			]
		});
